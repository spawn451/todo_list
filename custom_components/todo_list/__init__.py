...
class NoMatchingTodoListItem(Exception):
    """No matching item could be found in the todo list."""
    ...
    def async_reorder(self, item_ids, context=None):
        ...
        for key in all_items_mapping:
            ...
            if all_items_mapping[key]["complete"] is False:
                raise vol.Invalid(
                    "The item ids array doesn't contain all the unchecked todo list"
                    " items."
                )
...
@callback
@websocket_api.websocket_command({vol.Required("type"): "todo_list/items"})
def websocket_handle_items(
    hass: HomeAssistant,
    connection: websocket_api.ActiveConnection,
    msg: dict[str, Any],
) -> None:
    """Handle getting todo_list items."""
...
@websocket_api.websocket_command(
    {vol.Required("type"): "todo_list/items/add", vol.Required("name"): str}
)
...
async def websocket_handle_add(
    ...
    """Handle adding item to todo_list."""
    item = await hass.data[DOMAIN].async_add(msg["name"], connection.context(msg))
    connection.send_message(websocket_api.result_message(msg["id"], item))


@websocket_api.websocket_command(
    {vol.Required("type"): "todo_list/items/remove", vol.Required("item_id"): str}
)
@websocket_api.async_response
async def websocket_handle_remove(
    ...
    """Handle removing todo_list item."""
    msg_id = msg.pop("id")
...
@websocket_api.websocket_command(
    {
        vol.Required("type"): "todo_list/items/update",
...
async def websocket_handle_update(
    ...
    """Handle updating todo_list item."""
    msg_id = msg.pop("id")
    item_id = msg.pop("item_id")
    msg.pop("type")
    data = msg
...
@websocket_api.websocket_command({vol.Required("type"): "todo_list/items/clear"})
@websocket_api.async_response
